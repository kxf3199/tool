...
跳转控制
 */
function ctrl_go_page(param){
  //console.log(param);   
  if(param.value=='')
    param.value=param.defaultValue;
  else if(!isNaN(param.value))
    $("#go_page_num").attr("href","#"+param.value);  
...
function table(options){
  ...
  this.pagination = function(tdCell){
    ...
    //跳转
    var oA = $("<input/>");    
    oA.attr("id","input_page_num");
    oA.attr("value","跳转页面");
    oA.attr("style","width:50px;");
    oA.attr("onfocus","if(this.value==defaultValue){this.value=''}");
    oA.attr("onblur","ctrl_go_page(this)");
    $td.append(oA);
    var oA = $("<a/>");
    oA.attr("id","go_page_num");
    oA.attr("href","#" + this.maxPageNum);
    oA.html("go");
    $td.append(oA);
    ...
    page_a.unbind("click").bind("click",function(){
      //跳转判断
      if($(this).attr('id')=="go_page_num")
      {
        var goPageNum = parseInt($(this).attr('href').substring(1));
        tempThis.groupNum = goPageNum/tempThis.groupDataNum;
        tempThis.currentPageNum = goPageNum;
        tempThis.init((tempThis.currentPageNum-1)*tempThis.displayNum,
        tempThis.currentPageNum*tempThis.displayNum);
        $("#input_page_num").attr("value",tempThis.currentPageNum);
      return false;
...
function appendDataToTable(tableObj,data,func)
{
    if (!tableObj) {
        var err="没有表对象";
        func(err,tableObj);
    ...
    data.forEach(function(d){
      tableObj.content.push(d);
    }); 
    tableObj.allDataNum+=data.length;
    func("",tableObj);
}